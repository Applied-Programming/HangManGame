...
namespace HangMangame
{
    public partial class Form1 : Form
    {
        ...
        void DrawHangPost()
        {
            ...
            gr.DrawLine(pen, new Point(130, 218), new Point(130, 5));
            gr.DrawLine(pen, new Point(135, 5), new Point(65, 5));
            gr.DrawLine(pen, new Point(60, 0), new Point(60, 50));
            /*
        ...
        void DrawBodyParts(BodyParts bp)
        {
            ...
            if (bp == BodyParts.Head)
                g.DrawEllipse(pen, 40, 50, 40, 40);
            else if (bp == BodyParts.Left_Eye)
            {
                SolidBrush sb = new SolidBrush(Color.Black);
                gr.FillEllipse(sb,50,60,5,5);
            }
            else if (bp == BodyParts.Right_Eye)
            {
                SolidBrush sb = new SolidBrush(Color.Black);
                gr.FillEllipse(sb, 63, 60, 5, 5);
            }
            else if (bp == BodyParts.Mouth)
            {
                gr.DrawArc(pen, 50, 60, 20, 20, 45, 90);    
            }
            else if (bp == BodyParts.Body)
            {
                gr.DrawLine(pen, new Point(60, 90), new Point(60, 170));
            }
            else if (bp == BodyParts.Left_Arm)
            {
                gr.DrawLine(pen, new Point(60, 100), new Point(30, 85));
            }
            else if (bp == BodyParts.Right_Arm)
            {
                gr.DrawLine(pen, new Point(60, 100), new Point(90, 85));
            }
            else if (bp == BodyParts.Left_Leg)
            {
                gr.DrawLine(pen, new Point(60, 170), new Point(30, 190));
            }
            else if (bp == BodyParts.Right_Leg)
            {
                gr.DrawLine(pen, new Point(60, 170), new Point(90, 190));
            }
        }
